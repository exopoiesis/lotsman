...
- **M1** — **Marina + Lotsman QE + Lotsman CP2K**, single-host routing. Core API: `host_add` + `run` + `status` + `kill` + `logs` + `harvest essential`. Marina-as-MCP-server для Claude Code, Marina-as-MCP-client к Lotsman через ssh stdio. End-to-end на одном QE и одном CP2K инстансе. **Validates Marina-as-proxy + cross-tool API contract** одним заходом.
